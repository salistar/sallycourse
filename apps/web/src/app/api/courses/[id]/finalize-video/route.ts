...
export async function POST(
  ...
  if (lessonIds.length === 0) {
    return NextResponse.json(
      { error: "Aucune leçon approuvée — validez d'abord l'aperçu rapide de chaque leçon.", code: 'noApprovedLesson' },
      { status: 409 },
    );
  }
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Impossible de lancer la version finale, réessayez plus tard.', code: 'finalVersionLaunchFailed' },
      { status: 503 },
    );
  }
